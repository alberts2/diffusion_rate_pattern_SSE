...
in_fp  = paste0(fp,"data/Simulation/no_rejection_states/fast_rates_40percent_miss/combined_fast/")
...
# create data frame for storing information from each tree in this fast_rate dataset
treeset_fast_df = data.frame(tree = rep(NA,total_sim), tip_count = rep(NA,total_sim), 
...
for (t in 1:length(tree_sizes)){ #loop over each tree size 
  for (i in 1:num_sim){ #loop over each tree with the given tree size 
    ...
    # load the tree 
    tree_dat   = read.tree(paste0(in_fp,"tree_fast_",tree_sizes[t],"taxa_",i,".tre"))
    # add information about the tree to data_frame
    treeset_fast_df$tree[num_sim*(t-1)+i]      = paste0("tree ", i, " with size ", tree_sizes[t])
    treeset_fast_df$tip_count[num_sim*(t-1)+i] = length(tree_dat$tip.label)
    treeset_fast_df$root_age[num_sim*(t-1)+i]  = max(node.depth.edgelength(tree_dat))
    treeset_fast_df$sim_type[num_sim*(t-1)+i]  = "fast"
    # load character data at tips after pruning
    char_dat   = read.table(paste0(in_fp,"tipstates_fast_",tree_sizes[t],"taxa_",i,".csv"),sep=";",header = T)
    # load the character states before pruning
    full_char  = read.table(paste0(in_fp,"all_tipstates_fast_",tree_sizes[t],"taxa_",i,".csv"),sep=";",header = T)
    ...
    treeset_fast_df$freq1_obs[num_sim*(t-1)+i] = 1-treeset_fast_df$freq0_obs[num_sim*(t-1)+i]
    # convert to a vector
    ...
    true_param = read.table(paste0(in_fp,"pars_fast_",tree_sizes[t],"taxa_",i,".csv"),sep=";",header = T)
    # add true generating param to dataframe
    treeset_fast_df$lambda0_truth[num_sim*(t-1)+i] = true_param$lambda0
    treeset_fast_df$lambda1_truth[num_sim*(t-1)+i] = true_param$lambda1
    treeset_fast_df$mu0_truth[num_sim*(t-1)+i]     = true_param$mu0
    treeset_fast_df$mu1_truth[num_sim*(t-1)+i]     = true_param$mu1
    treeset_fast_df$q01_truth[num_sim*(t-1)+i]     = true_param$q01
    treeset_fast_df$q10_truth[num_sim*(t-1)+i]     = true_param$q10
    # create bisse object for that tree
    bisse_class = make.bisse(tree_dat,char_dat_mod,strict = FALSE,sampling.f = c(prop_0,prop_1))
    # fit bisse 
    ...
    # added the optima parameters to treeset_fast_df
    treeset_fast_df$lambda0_est[num_sim*(t-1)+i] = params[1]
    treeset_fast_df$lambda1_est[num_sim*(t-1)+i] = params[2]
    treeset_fast_df$mu0_est[num_sim*(t-1)+i]     = params[3]
    treeset_fast_df$mu1_est[num_sim*(t-1)+i]     = params[4]
    treeset_fast_df$q01_est[num_sim*(t-1)+i]     = params[5] 
    treeset_fast_df$q10_est[num_sim*(t-1)+i]     = params[6]
    # added theoretical stationary frequency using the optimal params 
    treeset_fast_df$freq0_statio[num_sim*(t-1)+i] = compute_statio(params)
    treeset_fast_df$freq1_statio[num_sim*(t-1)+i] = 1-treeset_fast_df$freq0_statio[num_sim*(t-1)+i]
    # compute abs distance between obs_0 and statio_0 for this tree
    treeset_fast_df$distance_to_statio_0[num_sim*(t-1)+i] = abs(treeset_fast_df$freq0_obs[num_sim*(t-1)+i]-treeset_fast_df$freq0_statio[num_sim*(t-1)+i])
  }
}

# save output
write.table(treeset_fast_df,paste0(out_fp,"bisse_fast_miss40_summary_no_rejection_sampling.csv"),sep = ";",row.names = F)
